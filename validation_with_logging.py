...
class KFoldCrossValidation(ValidationStrategy):
    ...
    """
    def __init__(self, model, k=10, names=None):
        """
        Args:
    ...
    def validate(self, X, y, description="ExperimentName"):
        ...
        for i in range(0,self.k):
            ...
            for j in testIdx:
                ...
                success = prediction == y[j]
                if success:
                    true_positives = true_positives + 1
                else:
                    false_positives = false_positives + 1
                if self.names is not None:
                    self.logger.info("{0}Expected {1} and obtained {2}".format("Success: " if success else "   Fail: ", self.names[y[j]], self.names[prediction]))
